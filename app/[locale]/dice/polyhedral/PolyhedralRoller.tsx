    ...
    <RollerLayout>
      <RollerLayoutContent>
        ...
        <RollerLayoutControlArea className="col-span-2">
          {/* Wait for hydration before rendering config-dependent maxQuantity */}
          {hasHydrated && (
            <DiceSelectWheel
              max={maxQuantity}
            ...
            <div className="flex gap-2">
              {config.enabledDice.map((diceType) => (
                <Button
                  key={diceType}
                  variant={
                    selectedDiceType === diceType ? 'default' : 'outline'
                  }
                  size="sm"
                  onClick={() =>
                    setSelectedDiceType(diceType as PolyhedralDiceType)
                  }
                  className={
